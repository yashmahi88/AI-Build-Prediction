...
#             rule['source_metadata'] = doc_metadata if doc_metadata else {}
#             rule['citation'] = build_citation(doc_metadata) if doc_metadata else None
#         rules.append(rule)
...
def build_citation(metadata: dict) -> str:
    """Build citation string from document metadata"""
    if not metadata:
        return None
    ...
    if confluence_url:
        ...
        if '/' in source:
            ...
            page_name = 'Confluence'
        return f" {page_name} - {confluence_url}"
    
    # JENKINS LOG SOURCES
    source = metadata.get('source', '')
    if 'jenkins' in source.lower() or 'dynamic_knowledge' in source:
        parts = source.split('/')
        filename = parts[-1] if parts else 'log'
        
        # Try to extract job name and build number
        match = re.search(r'([a-zA-Z0-9_-]+)-(\d+)', filename)
        if match:
            job_name = match.group(1)
            build_num = match.group(2)
            return f" Jenkins: {job_name} #{build_num} - http://localhost:8080/job/{job_name}/{build_num}"
        
        return f" Jenkins: {filename}"
...
def extract_platform_agnostic_rules(content: str, source_type: str = "documentation") -> List[Dict]:
    ...
    # Your existing rule extraction methods...
    rules.extend(extract_linguistic_rules(content))
    rules.extend(extract_structural_rules(content))
    rules.extend(extract_conditional_rules(content))
    rules.extend(extract_comparison_rules(content))
    rules.extend(extract_prerequisite_rules(content))
    
    # Return rules as dicts if they aren't already
    formatted_rules = []
    for rule in rules:
        if isinstance(rule, str):
            # Convert string rules to dicts
            ...
            # Ensure dict rules have required fields
            if 'rule_text' not in rule:
                rule['rule_text'] = str(rule)
...
def generate_intelligent_structured_response(pipeline_text, all_rules, rule_analysis, success_patterns, failure_patterns):
    """Generate response WITH sources section at the end"""
    
    technologies = detect_technology_stack(pipeline_text)
    
    response = f"""DETECTED_STACK: {', '.join(technologies) if technologies else 'Unknown'}
...
"""
    ...
    for rule in all_rules[:20]:
        rule_count += 1
        if isinstance(rule, dict):
            rule_text = rule.get('rule_text', str(rule))[:120]
            confidence = rule.get('confidence', 0.8)
            response += f"• {rule_text}... - PASS (conf: {confidence})\n"
        else:
            response += f"• {str(rule)[:120]}...\n"
    ...
    response += f"SATISFIED_RULES: {rule_analysis.get('satisfied_count', 0)}\n"
    response += f"VIOLATED_RULES: {rule_analysis.get('violated_count', 0)}\n"
    
    if rule_analysis.get('risk_factors'):
        response += f"RISK_FACTORS: {', '.join(rule_analysis['risk_factors'])}\n"
    
    # Prediction
    prediction_result, confidence, reasoning = make_intelligent_prediction(
        pipeline_text, all_rules, rule_analysis
    )
    ...
    # Extract sources from vectorstore
    try:
        docs = vectorstore.similarity_search(pipeline_text, k=10)
        unique_sources = set()
        ...
        for doc in docs:
            ...
            if confluence_url and confluence_url.strip():
                ...
                unique_sources.add(f" {page_name} - {confluence_url}")
            
            # Also add Jenkins sources
            source = doc.metadata.get('source', '')
            if 'jenkins' in source.lower():
                ...
                if match:
                    job_name = match.group(1)
                    build_num = match.group(2)
                    unique_sources.add(f" Jenkins: {job_name} #{build_num} - http://localhost:8080/job/{job_name}/{build_num}")
        
        # Add sources to response
        if unique_sources:
            response += f"\n{'='*60}\n"
            response += "SOURCES & REFERENCES:\n"
            response += f"{'='*60}\n"
...
def enhanced_rag_analysis_with_intelligent_rules(pipeline_text: str) -> str:
    """Enhanced RAG analysis using platform-agnostic rule extraction WITH METADATA"""
    ...
    for doc in docs:
        ...
        metadata = doc.metadata  #  Get FULL metadata
        
        # Determine source type from metadata
        source = metadata.get('source', '')
        if 'static_knowledge' in source or metadata.get('confluence_url'):
            source_type = 'confluence'
        elif 'jenkins' in source.lower() or 'dynamic_knowledge' in source:
            source_type = 'jenkins'
        else:
            source_type = metadata.get('category', 'documentation')
        
        # Extract intelligent rules
        intelligent_rules = extract_platform_agnostic_rules(content, source_type)
        
        #  ADD METADATA TO EACH RULE - THIS IS THE KEY!
        for rule in intelligent_rules:
            if isinstance(rule, dict):
                rule['source_metadata'] = metadata  # Attach metadata!
                rule['citation'] = build_citation(metadata)  # Build citation
            all_intelligent_rules.append(rule)
...
"""
    ...
    # ✅ NEW: Add SOURCES & REFERENCES section
    try:
        print(" Extracting unique sources from vectorstore...")
        all_sources_set = set()
        
        # Get top docs for sources
        ...
        for doc in docs:
            ...
            if confluence_url and confluence_url.strip():
                source = doc.metadata.get('source', '')
                page_name = source.split('/')[-1].replace('.md', '').replace('_', ' ') if '/' in source else 'Confluence'
                all_sources_set.add(f" {page_name} - {confluence_url}")
            
            # Extract Jenkins sources
            source = doc.metadata.get('source', '')
            if 'jenkins' in source.lower():
                parts = source.split('/')
                filename = parts[-1] if parts else 'log'
                ...
                if match:
                    job_name = match.group(1)
                    build_num = match.group(2)
                    all_sources_set.add(f" Jenkins: {job_name} #{build_num} - http://localhost:8080/job/{job_name}/{build_num}")
...
============================================================
"""
            for i, source in enumerate(sorted(all_sources_set), 1):
                response += f"{i}. {source}\n"
            print(f"✅ Added {len(all_sources_set)} sources to response")
...
def analyze_pipeline_workspace_dynamically(pipeline_content: str) -> Dict:
    ...
    print(f"   Configurations: {len(analysis['configuration_files'])} detected")
    print(f"   Build commands: {len(analysis['build_files'])} detected")
